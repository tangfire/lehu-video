...
func NewUserUsecase(base BaseAdapter, core CoreAdapter, authSecret string, logger log.Logger) *UserUsecase {
	if authSecret == "" {
...
func (uc *UserUsecase) GetCompleteUserInfo(ctx context.Context, input *GetUserInfoInput) (*GetUserInfoOutput, error) {
	...
	// 2. 聚合数据
	user := uc.aggregateUserInfo(baseInfo, nil, nil)
...
func (uc *UserUsecase) SearchUsers(ctx context.Context, input *SearchUsersInput) (*SearchUsersOutput, error) {
	...
	for _, baseUser := range baseUsers {
		user := uc.aggregateUserInfo(baseUser, nil, nil)
		// 搜索结果中隐藏隐私字段
		user.Mobile = ""
		user.Email = ""

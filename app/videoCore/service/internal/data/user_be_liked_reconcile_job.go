...
func (j *UserBeLikedReconcileJob) RunOnce(ctx context.Context) error {
	...
	if err := j.db.WithContext(ctx).Raw(sqlCount, sinceTime).Row().Scan(&minID, &maxID); err != nil {
		j.log.Errorf("获取用户 ID 范围失败：%v", err)
		return err
	}

	if minID == 0 && maxID == 0 {
		j.log.Infof("最近 %d 天内无视频更新，跳过对账", j.sinceDays)
		return nil
	}
	...
	j.log.Infof("用户获赞数对账完成，处理 %d 个用户，更新 %d 条记录，耗时 %v", totalUsers, totalAffected, time.Since(start))
	return nil
}

...
审核链路做了体验和成本保护：AI 模式下普通用户帖子会先作者可见并进入异步 Agent 初审，本地规则只负责风险分级和高风险兜底。Agent 高置信低风险才自动公开；中风险、不确定、高风险或低置信内容保持作者可见、公共不可见，并推飞书或进入后台人工处理。模型预算超限或 Agent 不可用时，规则低风险会兜底公开，其他内容转人工。
...
官方账号只通过 `CAMPUS_EZAI_BOT_USER_ID` / `CAMPUS_EZAI_USER_ID` 配置。后台“回复状态”页只控制 `ezai_auto_reply_enabled`：开启时 AI 正常接管；关闭、模型未配置、预算跳过或任务最终失败时，会给官方账号发 `mention` 类型站内通知，让运营手动回复。

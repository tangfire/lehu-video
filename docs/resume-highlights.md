...
- 新增运营 Copilot Agent：独立 `campus-agent` 服务使用 LangGraph 编排工具调用流程，读取后台统计、审核队列、RAG 日志和安全数据，生成只读巡检、治理建议和知识库缺口分析。
...
- Agent 采用 human-in-the-loop 边界：LangChain tool 只封装只读接口，高风险写操作仍由运营在后台确认执行，兼顾展示 Agent 能力和生产安全。

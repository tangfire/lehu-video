...
- `CAMPUS_AI_EZAI_ENABLED` 没有被设置为 `false/off/disabled`，且后台“e仔自动回复”开关开启。

官方账号身份只从 `CAMPUS_EZAI_BOT_USER_ID` / `CAMPUS_EZAI_USER_ID` 读取，不在后台选择。没满足这些条件时，e仔自动回复不会创建任务；如果已经配置官方账号，用户 `@e仔` 会转成给官方账号的站内通知，让运营用这个账号人工回复，不影响社区其他功能。
...
sequenceDiagram
    ...
    else 自动回复关闭、预算跳过或任务最终失败
        API->>DB: 给官方账号创建 mention 通知
    end
```

触发条件：

- 评论内容包含 `@深汕e仔`、`＠深汕e仔`、`@e仔`、`＠e仔`，或当前后台人设名称。
- 发送评论的人不是 e仔官方账号。
- 官方账号由环境变量配置；后台只控制“e仔自动回复”是否开启。

幂等设计：

- `campus_ai_reply_task.trigger_comment_id` 有唯一索引，防止同一评论重复创建任务。
- 生成的 e仔回复评论 ID 使用任务 ID，重复处理时能识别已有回复。
- 人工接管通知使用同一触发评论的 dedupe key，避免任务重试时刷屏。
...
| 场景 | 结果 |
| --- | --- |
| 没有配置 bot 用户 | 不创建 e仔回复任务，也不发送人工通知 |
| 关闭后台“e仔自动回复”或没有配置模型 key | 不创建 e仔回复任务，给官方账号发站内通知 |
...
| RAG 服务异常 | 记录错误；如果模型可用，会尽量按帖子上下文回答 |
| 模型接口异常或超时 | 任务重试；最终失败后通知官方账号人工接管 |
| 达到预算或每日回复上限 | 不强行调用模型，通知官方账号人工接管 |
...
| `PUT /v1/campus/admin/ai-replies/settings` | 开启或关闭 e仔自动回复 |

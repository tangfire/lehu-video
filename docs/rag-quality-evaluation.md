...
flowchart TD
    ...
    C --> Draft[Agent 自动沉淀停用草稿]
    Draft --> D[人工确认后加入 RAG 评测集]
    E[人工新增黄金问题] --> D
    D --> F[批量运行评测]
    F --> G[查看通过率/平均分/失败样例]
...
为了让评测集自然长出来，`campus-api` 的后台任务会每天扫描真实 e仔/RAG 记录，把这些问题自动沉淀成 `status=0` 的停用草稿：

- `quality_label in wrong,needs_fix,unsafe`
- `need_knowledge=true` 且 `confidence <= 0.52`
- e仔失败任务中能提取到明确用户问题的记录

草稿会绑定 `source_log_id`，同一条真实日志不会重复创建。后台“RAG评测”页可以筛选“Agent 草稿”，运营确认有价值后批量启用。这样做的好处是：系统负责把问题捞出来，人负责判断是否纳入回归集，不会让低质量自动数据直接污染评测标准。

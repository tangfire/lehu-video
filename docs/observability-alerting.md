...
flowchart LR
    ...
    API[campus-api] -->|运营通知 /agent| Alert
    Alert --> Feishu[飞书群机器人]
```
...
- 收手机通知：Grafana Alerting 和运营通知都通过 `alert-webhook` 发到飞书群；运营通知包括 Agent 报告，也包括不调用模型的举报、反馈和 SLA 提醒。
...
| `alert-webhook` | 接收 Grafana 告警和运营通知，转换为飞书消息 | `deploy/observability/alert-webhook/alert_webhook.py` |
...
| 运营通知 | `campus-api` | 每日运营日报、高风险运营提醒、举报/重要反馈提醒、待人工审核提醒、手动发送的 Agent 结果 | 运营处理 |
...
- 飞书运营通知负责 Agent 运营摘要、举报/反馈提醒和待审核帖子的人机确认；删帖、封禁、举报处理仍由 `campus-api` 校验后完成，运营后台是最终事实入口。

...
> 基于 Go Kratos + gRPC + Consul 构建校园社区轻量微服务系统，拆分 API 网关、账号文件服务、用户资料服务和 AI/RAG 服务；使用 Redis 缓存热点读和限流，COS/CDN 承载公开媒体，Grafana + Loki + Prometheus + Alloy 实现日志搜索、健康监控和飞书告警。
